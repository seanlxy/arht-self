<?php 
	...
	{	
		...
		$email_template_tags['email_subject'] = 'You have received a new enquiry from the website.';
		$email_template_tags['root']          = $htmlrootfull;
		$email_template_tags['company']       = $company_name;
		...
		if( $email_template )
		{
			...
			if( $company_email )
			{
				...
				if( $mail->Send() )
				{
					header("Location: {$htmlrootfull}/{$page}/{$segment1}?success=".md5($new_enquiry));
					exit();
				}
