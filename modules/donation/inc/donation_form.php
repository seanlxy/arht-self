...
$donation_types = fetch_row("SELECT dt.`id` AS donation_type_id, dt.`page_meta_data_id`, dt.`icon_cls`,
	dt.`is_promoting`, pmd.`name`, pmd.`status`, dt.`id` AS `page_id`,
	pmd.`full_url`, pmd.`heading`, pmd.`introduction`
...
if ($segment1 == 'become-a-sponsor' || $segment1 == 'give-monthly' ) 
{
    $heading_monthly = '<h1 class="section__heading section__heading--normal">Choose your monthly gift</h1>';
}
...
if ($segment1 == 'become-a-sponsor' || $segment1 == 'give-monthly') 
{ 
    $dontation_btn_text = 'Give your monthly gift';
}
else
{
    $dontation_btn_text = 'Donate Now';
}
...
H;
    ...
    <section class="section section--white-bg">
            ...
            {$error}
                <div class="row donation-radio-group">
                    <div class="col-md-12 text-center">
                        <div class="donation-radio">
                            <input class="donation-radio__input" type="radio" id="r1" name="donation_radio" value="25" {$nzd25} />
                            <label class="donation-radio__label" for="r1">$25</label>
                        </div>

                        <div class="donation-radio">
                            <input class="donation-radio__input" type="radio" id="r2" name="donation_radio" value="50" {$nzd50} />
                            <label class="donation-radio__label" for="r2">$50</label>
                        </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="form-group">
                            <label></label>
                            <div class="controls">
                                <div class="g-recaptcha" data-sitekey="6Lc2Tk8UAAAAAPhje3JaiLVc8oHLcQhHVqmBw8o9"></div>
                                <span class="help-inline">{$captchaErrorMsg}</span>
                            </div>
                        </div> 
                        <input type="hidden" name="page_id" value="{$page_id}">
                        <input type="hidden" name="meta_page_id" value="{$page_meta_data_id}">
                        <input type="hidden" name="donation_type_id" value="{$donation_type_id}">           
                ...
                <div class="row">
                    <div class="col-md-12">
                        <button type="submit" name="submit" class="btn btn--full-width btn--red form__btn">{$dontation_btn_text}</button>
                    </div>
                </div>
            </form>
        </div>
...
if($segment1 == 'leave-a-legacy'){
    ...
    require_once("{$moddir}/contact/main.php");
    $tags_arr['mod_view'] .= $contact_form;
