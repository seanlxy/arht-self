...
function build_page_url($page_id, $reset = false)
{
    ...
    if($page_id)
    {
       $page_data = fetch_row("SELECT e.`page_meta_data_id`, pmd.`url`
            FROM `events` e
...
function save_item()
{
    ...
    $events_data['event_region']      = sanitize_input('event_region');
    $events_data['event_start_date']  = sanitize_input('event_start_date');
    $events_data['event_end_date']    = sanitize_input('event_end_date');

    
    
    update_row($events_data, 'events', "WHERE id = '{$id}' LIMIT 1");
