...
$emergencymode_settings = fetch_row("SELECT `emergency_mode`, `emergency_mode_msg` FROM `general_settings` WHERE `id` = '1' LIMIT 1");

if ($emergencymode_settings['emergency_mode'] == '1') 
{
	$emergencymode_settings['emergency_mode_msg_view'] = $emergencymode_settings['emergency_mode_msg'];
	echo process_template("templates/emergencymode.html", $emergencymode_settings, '==', '==');

	$request_url = $_SERVER['REQUEST_URI'];

	if (!empty($request_url) && ($request_url != '/')) 
	{
...
$new_redirect_details = fetch_row("SELECT `new_url`, `status_code` FROM `redirect` WHERE (REPLACE(`old_url`, '{$htmlrootfull}', '') = '$requested_url' OR REPLACE(`old_url`, '{$htmlroot_no_www}', '') = '{$requested_url}') AND `new_url` != `old_url` AND `status` = 'A' LIMIT 1");
